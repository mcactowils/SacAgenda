...
export default function App() {
  ...
  useEffect(() => {
    async function init() {
      try {
        // Load shared name groups (used by all users)
        let names = DEFAULT_NAMES;
        try {
          const r = await storage.get("shared-name-groups");
          if (r?.value) names = { ...DEFAULT_NAMES, ...JSON.parse(r.value) };
        } catch {}
        setNameGroups(names);

        // Load shared custom hymns (used by all users)
        let ch = {};
        try { const r = await storage.get("shared-custom-hymns"); if (r?.value) ch = JSON.parse(r.value); } catch {}
        setCustomHymns(ch);
        ...
        let userList = [];
        try { const r = await storage.get("agenda-users"); if (r?.value) userList = JSON.parse(r.value); } catch {}
        setUsers(userList);

        // Load smart text
        let text = DEFAULT_SMART_TEXT;
        try { const r = await storage.get("agenda-smart-text"); if (r?.value) text = JSON.parse(r.value); } catch {}
        setSmartText(text);

        // Check for persistent session
        try {
          const r = await storage.get("current-session");
          if (r?.value) {
            const session = JSON.parse(r.value);
            const user = userList.find(u => u.username === session.username);
  ...
  const saveNameGroups = useCallback(async (g) => {
    setNameGroups(g);
    try { await storage.set("shared-name-groups", JSON.stringify(g)); } catch {}
  }, []);

  const saveCustomHymns = useCallback(async (h) => {
    setCustomHymns(h);
    try { await storage.set("shared-custom-hymns", JSON.stringify(h)); } catch {}
  }, []);
  ...
  const loginUser = useCallback(async (username) => {
    try {
      ...
      await storage.set("current-session", JSON.stringify({ username: user.username, timestamp: Date.now() }));
